...
ENTRYPOINT youtube-dl --newline --continue --fixup warn --retries 5 --max-downloads ${maxdownloads} --format ${format} --match-filter "!is_live" --output '%(uploader)s/%(upload_date)s-%(title)s-%(id)s.%(ext)s' ${url}